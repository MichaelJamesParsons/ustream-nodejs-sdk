...
class RequestClient {
  ...
  constructor (apiEndpoint, http = null) {
    ...
    this.verbs = {
      ...
      'delete': {func: this.http.put, hasData: false}
    }
  }
   ...
   *
   * @param {string}   verb    - Type of request: get, post, put.
  ...
  requestRaw (verb, route, data = {}, headers = {}) {
    return new Promise((resolve, reject) => {
      if (!this.verbs.hasOwnProperty(verb)) {
        reject(new Error(`Unsupported HTTP verb ${verb}. Supported verbs are get, post, and put.`))
      }
