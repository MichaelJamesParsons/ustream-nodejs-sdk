...
class Playlist extends ApiResource {
   ...
   * @param {Number}    channelId
   * @param {Number}    is_enabled - Whether the playlist is enabled or not. Values: 1 (enabled), 0 (disabled). (1 by default)
   *
   * @returns {*}
   */
  create (channelId, title, is_enabled=1) {
    return this.context.authRequest('post', `/channels/${channelId}/playlists.json`, qs.stringify(title,is_enabled)).then((res) => {
