...
class Playlist extends ApiResource {
  ...
  list(channelId, pageSize = 50, page = 1, filter = 'include_empty') {
      return this.context.authRequest('get', `/channels/${channelId}/playlists.json?pagesize=${pageSize}&page=${page}&filter=${filter}`)
        .then((res) => {
  ...
  listVideos(playlistId, pageSize = 50, page = 1) {
      return this.context.authRequest('get', `/playlists/${playlistId}/videos.json?pagesize=${pageSize}&page=${page}`).then((res) => {
