...
class Channel extends ApiResource {
   ...
   */
  get (channelId, opts = {}) {
    return this.context.authRequest('get', `/channels/${channelId}.json`,
      qs.stringify(opts)).then((res) => {
  ...
  create (title, opts = {}) {
    opts.title = title
    return this.context.authRequest('post', '/users/self/channels.json',
      qs.stringify(opts)).then((res) => {
      return Promise.resolve(res.channel)
    })
  }
  ...
  list (pageSize = 100, page = 1) {
    ...
    return this.context.authRequest('get', `/users/self/channels.json`,
